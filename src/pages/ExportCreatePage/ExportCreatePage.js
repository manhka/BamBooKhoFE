...
const ExportCreatePage = () => {
    ...
    useEffect(() => {
        const loadCustomers = async () => {
            ...
            setCustomers(customerData.map(c => ({ value: c.CustomerID, label: `${c.CustomerName} (${c.Phone || 'N/A'})` })) || []);
        };
        loadCustomers();
    }, []);
    ...
    const handleAddProduct = (selectedOption) => {
        if (selectedOption?.product) {
            const existingIndex = details.findIndex(d => d.product.BarcodeProduct === selectedOption.product.BarcodeProduct);
            if (existingIndex === -1) {
                setDetails([...details, {
                ...
                handleDetailChange(existingIndex, 'quantity', details[existingIndex].quantity + 1);
            }
    ...
    const calculateTotal = () => {
        return details.reduce((sum, item) => (sum + (Number(item.product.SalePrice) || 0) * (Number(item.quantity) || 0)), 0);
    };
    ...
    const handleCustomerUpdated = (updatedCustomer) => {
        setIsEditModalOpen(false);
        const newLabel = `${updatedCustomer.CustomerName} (${updatedCustomer.Phone || 'N/A'})`;
        ...
        toast.success("Cập nhật thông tin khách hàng thành công!", { autoClose: 3000 });
    };
    ...
    const handleSubmit = async (event) => {
        ...
        // Validation Frontend
        if (!selectedCustomer) { currentErrors.customer = "Vui lòng chọn khách hàng."; formIsValid = false; }
        if (details.length === 0) { currentErrors.details = "Vui lòng thêm ít nhất một sản phẩm."; formIsValid = false; }

        details.forEach((item, index) => {
            if (!item.quantity || item.quantity <= 0) { currentErrors[`details[${index}].quantity`] = "Số lượng phải > 0."; formIsValid = false; }
            else if (item.quantity > item.product.NumberOfProduct) { currentErrors[`details[${index}].quantity`] = `Vượt tồn kho (${item.product.NumberOfProduct}).`; formIsValid = false; }
        });
                    ...
                    err.errors.forEach(e => {
                        if (e.rowIndex !== undefined) {
                            Object.entries(e.errors).forEach(([field, msg]) => {
                                backendErrors[`details[${e.rowIndex - 1}].${field.toLowerCase()}`] = msg;
                            });
                        } else if (e.general) { backendErrors.general = e.general; }
                    });
                } else if (typeof err.errors === 'object') {
                    Object.entries(err.errors).forEach(([field, msg]) => { backendErrors[field.toLowerCase()] = msg; });
                }
                setErrors(backendErrors);
            } else { setErrors({ general: err.message || 'Lỗi khi tạo phiếu xuất.' }); }

            // === SỬA: HIỆN TOAST LỖI ===
            const errorMessage = (backendErrors.general) || err.message || 'Lỗi không xác định khi tạo phiếu xuất.';
        ...
        <div className={styles.container}>
            ...
            {errors.general && (
                <div className={styles.alert}>
                    <AlertTriangle size={18} className={styles.alertIcon} /> {errors.general}
                </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <div className={styles.customerInputWrapper}>
                                <Select
                                    ...
                                    styles={{ 
                                        control: (base) => (errors.customer ? { ...base, borderColor: 'red', zIndex: 10 } : { ...base, zIndex: 10 }),
                            ...
                            {errors.customer && <small className={styles.errorMessage}>{errors.customer}</small>}
                        </div>
                        <div className={styles.formGroup}>
                            <label htmlFor="exportDate" className={styles.formLabel}>Ngày Xuất</label>
                            <DatePicker
                                id="exportDate"
                ...
                <div className={styles.card}>
                    ...
                    <div className={styles.cardBody}>
                        <label htmlFor="productSearch" className={styles.formLabel}>Tìm Sản Phẩm (theo tên hoặc mã)</label>
                        <Select
                            id="productSearch"
                            ...
                            noOptionsMessage={() => productSearchInput.length < 2 ? "Gõ ít nhất 2 ký tự" : "Không tìm thấy sản phẩm"}
                            loadingMessage={() => "Đang tìm..."}
                            menuPortalTarget={document.body}
                            styles={{ 
                                control: (base) => (errors.details && details.length === 0 ? { ...base, borderColor: 'red' } : base),
                                menuPortal: (base) => ({ ...base, zIndex: 9999 })
                            }}
                        />
                        {errors.details && details.length === 0 && <small className={styles.errorMessage}>{errors.details}</small>}
                    </div>
                </div>
                ...
                <div className={styles.card}>
                    ...
                    <div className={styles.tableWrapper}>
                        <table className={styles.table}>
                            ...
                            <tbody>
                                {details.length === 0 ? (
                                    <tr><td colSpan="8" className={styles.emptyRow}>Chưa có sản phẩm nào</td></tr>
                                ) : (
                                    details.map((item, index) => (
                                        <tr key={item.product.BarcodeProduct}>
                                            ...
                                            <td data-label="Giá Bán">{Number(item.product.SalePrice).toLocaleString('vi-VN')}₫</td>
                                            <td data-label="Số Lượng">
                                                <input
                                                    type="number" min="1" max={item.product.NumberOfProduct}
                                                    value={item.quantity}
                                                    onChange={(e) => handleDetailChange(index, 'quantity', e.target.value)}
                                                    className={`${styles.formInput} ${styles.inputSmall} ${errors[`details[${index}].quantity`] ? styles.isInvalid : ''}`}
                                                />
                                                {errors[`details[${index}].quantity`] && <small className={styles.errorMessage}>{errors[`details[${index}].quantity`]}</small>}
                                            </td>
                                            <td data-label="BH (tháng)">
                                                <input
                                                    type="number" min="0" step="1" placeholder="Số tháng"
                                                    value={item.warrantyTime ?? ''}
                                                    onChange={(e) => handleDetailChange(index, 'warrantyTime', e.target.value)}
                                                    className={`${styles.formInput} ${styles.inputSmall} ${errors[`details[${index}].warrantyTime`] ? styles.isInvalid : ''}`}
                                                />
                                                {errors[`details[${index}].warrantyTime`] && <small className={styles.errorMessage}>{errors[`details[${index}].warrantyTime`]}</small>}
                                            </td>
                                            <td data-label="Ngày Bắt Đầu BH">
                                                <DatePicker
                                                    selected={parseDateString(item.warrantyStartTime)}
                                                    onChange={(date) => handleDetailChange(index, 'warrantyStartTime', date)}
                                                    wrapperClassName={styles.datePickerWrapper} 
                                                    className={`${styles.formInput} ${errors[`details[${index}].warrantyStartTime`] ? styles.isInvalid : ''}`}
                                                    dateFormat="dd/MM/yyyy"
                                                    placeholderText="Chọn ngày"
                                                    autoComplete="off"
                                                    isClearable 
                                                />
                                                {errors[`details[${index}].warrantyStartTime`] && <small className={styles.errorMessage}>{errors[`details[${index}].warrantyStartTime`]}</small>}
                                            </td>
                                            <td data-label="Thành Tiền">{((Number(item.product.SalePrice) || 0) * (Number(item.quantity) || 0)).toLocaleString('vi-VN')}₫</td>
                                            <td data-label="Xóa">
                                                <button type="button" className={`${styles.button} ${styles.buttonDangerOutline}`} onClick={() => handleRemoveProduct(index)}>
                                                    <Trash2 size={16} />
                                                </button>
                                            </td>
                                        </tr>
                            ...
                            {details.length > 0 && (
                                <tfoot>
                                    <tr className={styles.tableFooter}>
                                        ...
                                        <td colSpan="2">{calculateTotal().toLocaleString('vi-VN')}₫</td>
                                    </tr>
                                </tfoot>
                            )}
                        </table>
                ...
                <button
                    ...
                    disabled={isLoading || details.length === 0 || !selectedCustomer || Object.keys(errors).length > 0}
                >
                    {isLoading ? (
                        <>
                        ...
                        <><Send size={18} className={styles.buttonIcon} /> Hoàn Thành Phiếu Xuất</>
                    )}
                </button>
            </form>
