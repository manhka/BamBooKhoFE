    ...
    <div className={styles.container}>
      ...
      <div className={styles.card}>
        <div className={styles.cardHeader}>
          <Filter size={16} className={styles.iconFix} /> Bộ Lọc
        </div>
      ...
      <div className={styles.toolbar}>
        <div className={styles.sortContainer}>
          <label htmlFor="sortOrderSelect" className={styles.sortLabel}>
            Sắp xếp:
          </label>
          <select
            id="sortOrderSelect"
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan="5" className={styles.loadingRow}>
                  <span className={styles.spinner} role="status"></span> Đang
                  tải...
                </td>
              </tr>
                ...
                <tr key={order.ImportID}>
                  ...
                  <td data-label="Sản Phẩm">
                    {order.ImportDetails?.map((d) => (
                      <div
                        key={d.ImportDetailID}
                        className={styles.productDetailItem}
                      >
                        {d.Product?.ProductName || d.BarcodeProduct} (
                        {d.Quantity} * {d.UnitPrice?.toLocaleString("vi-VN")}₫)
                      </div>
                    ))}
                    {(!order.ImportDetails ||
                      order.ImportDetails.length === 0) &&
                      "Không có chi tiết"}
                  </td>
