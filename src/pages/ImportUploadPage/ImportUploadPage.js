...
import { Upload, FileText, AlertCircle, Search, Download, Users } from 'lucide-react';
...
const getGeneralError = (err) => {
    ...
    if (Array.isArray(err) && err.length > 0 && err[0].general) return err[0].general;
...
const ImportUploadPage = () => {
    ...
    useEffect(() => {
        const loadSuppliers = async () => {
            try {
                ...
                toast.error("Không thể tải danh sách nhà cung cấp.", { autoClose: 3000 });
            }
        };
        loadSuppliers();
    }, []);
    ...
    const handleUpload = async (event) => {
        ...
        try {
            ...
            toast.success(result.message || "Nhập hàng thành công!", { position: "top-right", autoClose: 3000 });
            ...
            if (err.errors) {
                ...
                displayError = { general: err.message || "Đã xảy ra lỗi khi nhập hàng." };
            }

            setError(displayError);
            toast.error(getGeneralError(displayError), { position: "top-right", autoClose: 5000 });
    ...
    const renderErrors = () => {
        ...
        if (error.general) {
            return <div className={`${styles.alert} ${styles.alertDanger}`}><AlertCircle size={18} className={styles.iconFix}/> {error.general}</div>;
        }
        ...
        return <div className={`${styles.alert} ${styles.alertDanger}`}>{error}</div>;
    };
    ...
    return (
        <div className={styles.container}>
            <BackButton />
            <h2 className={styles.pageTitle}>Nhập Hàng từ File Excel</h2>
            ...
            <div className={`${styles.card} ${styles.lookupCard}`}>
                ...
                <div className={styles.cardBody}>
                    <h4 className={styles.referenceTitle}>Tra cứu Tham Chiếu (Reference Tools)</h4>
            ...
            <div className={styles.card}>
                ...
                <div className={styles.cardBody}>
                    ...
                    <div className={styles.infoBar}>
                        ...
                        >
                            <Download size={16} className={styles.buttonIcon}/> Tải file mẫu để nhập hàng
                        </button>
                    </div>
                    
                    <form onSubmit={handleUpload}>
                        <div className={styles.formGroup}>
                            <label htmlFor="excelFile" className={styles.formLabel}>Chọn File Excel</label>
                            <input
                                type="file"
                                ...
                                disabled={isLoading}
                            />
                            {selectedFile && <small className={styles.textMuted}><FileText size={14} className={styles.iconFix}/> {selectedFile.name}</small>}
                        </div>
